...
### 1.5L — 高級視覺實作 (Premium Visuals)
- [ ] T217 [US*] 實作層疊玻璃態 CSS Utility：在 `src/styles/glass-card.css` 中實作 blur + 3% noise + 1px linear-gradient specular border。
...
## 階段 3.5：User Story 8 — 筆記紀錄與靈感捕捉 (Priority: P2) 🖋️

**目標**：實作筆記 CRUD、持久化、多筆記列表與匯出功能。

- [ ] T035 [US8] 實作筆記資料結構與 IndexedDB 存取：完成 `noteStorage.ts` 與 `noteController.ts`。
- [ ] T036 [US8] 實作筆記編輯模式轉向：點擊鋼筆圖示觸發 `transition-mode: note`，切換今日面板 UI 為編輯器。
- [ ] T037 [US8] 實作自動儲存機制：監聽編輯器 `input` 事件，搭配 2s debounce 自動存入資料庫。
- [ ] T038 [US8] 實作單日多則筆記列表：支援在面板中切換查看不同則筆記，並提供「＋」按鈕新增。
- [ ] T039 [US8] 實作匯出功能：生成 `.json` 整月備份檔案下載。

---

## 階段 3.6：User Story 9 — 生活隨筆分享渲染 (Priority: P2) 📱

**目標**：產出具備農曆資訊與筆記摘要的分享內容。

- [ ] T095 [US9] 實作分享內容生成器：整合當前農曆、日期與筆記內容，產生字串或圖像數據。
- [ ] T096 [US9] 整合 Web Share API：點擊分享按鈕在行動端喚起系統分享清單。
- [ ] T097 [US9] 實作 Fallback 分享（桌面端）：若 Web Share 不適用，改為複製分享文字至剪貼簿並顯示 Toast。
...
- [ ] T109 數位高級感視覺驗證：依據 plan.md §數位高級感實作技術指南驗證下列細節：
  - 層疊玻璃：確認包含 blur + noise + 1px specular gradient border。
  - 呼吸動畫：確認 Hero 背景具備 60s 循環之極緩慢 scale 動畫。
  - 主題緩衝：確認色彩切換時陰影 (box-shadow/text-shadow) 同步緩動，無瞬間閃變。
  - 驗收：視覺效果符合「數位高級感」與「時間藝術品」之定位。
